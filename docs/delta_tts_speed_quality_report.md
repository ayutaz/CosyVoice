...
- **品質は「調整済みプロトコルで AR とほぼ同等、未知文では劣る」が正直な現状**。
  ボトルネックは音響品質ではなく**長さ(duration)推定**に特定済み
- 次の一手(優先度順):
  1. **モーラベースの長さ推定**(GPU 不要): `ja_frontend`(pyopenjtalk)で読みを取得して
     モーラ数を数え、`モーラ数 × トークン/モーラ定数`で target_len を決める。
     表記と読みの乖離問題を直接解消する
  2. 長さの複数候補生成 + 確信度による再ランキング
  3. 訓練データ拡張(8000h 計画)による根本的な頑健性向上
