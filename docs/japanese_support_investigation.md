...
## 7. Phase 3 本番学習と Phase 2 評価の結果 (2026-07-08)

### 7.1 本番学習 (vast.ai H100 SXM, $2.33/hr)

- データ: moe-speech-plus 473 zip / 326GB → MOS≥1.5 + cross-CER≤0.2 フィルタで **289,893 発話 (~470h) 採用**(73.5%)
- 学習: llm のみ 5 エポック / 17,616 ステップ、**実行約 2 時間**(bf16 + torch.compile + max_frames 30000、VRAM 31.8/80GB)
- CV loss 3.36 → 3.22、エポック 3→4 の改善 ~0.008 で平坦化(5 エポック打ち切りが適切)
- 成果物: `checkpoints/cosyvoice3_ja/llm.pt`(CV ベスト 5 checkpoint 平均)。総費用 ~$25(前処理の試行錯誤込み)
- **バックアップ: HF private リポジトリ `ayousanz/cosyvoice3-ja-llm`**(llm.pt / ベスト単体 / 学習ログ。データセットライセンスが再配布禁止のため private 維持必須)

#### 本番前処理の運用ノート(8000h 実行時の必読事項)

実運用で踏んだ問題と確立した対処。コードはすべて反映済みだが、レンタル環境の運用判断はここに残す:

| 事象 | 対処(確立済み) |
|---|---|
| **hf_transfer が大容量データセット DL で無音ハング**、xet バックエンドは 429 レート制限 | `HF_HUB_DISABLE_XET=1` + `HF_HUB_ENABLE_HF_TRANSFER=0` + バックオフ付きリトライループ(prod スクリプト参照)。snapshot_download は resume が効く |
| vast.ai ホストの公称帯域は信用できない(公称 4.3Gbps で実効 0.4MB/s の例) | 起動直後に `/proc/net/dev` の RX レートを実測し、遅いホストは即破棄して乗り換える |
| **OpenMP スレッド爆発**: マルチプロセス前処理で各プロセスが全コア分の OMP スレッドを生成(14 proc×128 = ~1800 スレッド)し、全体が失速 + 新規プロセスの spawn も失敗 | ワーカープロセスには必ず `OMP_NUM_THREADS=2〜4` を設定 |
| **単一プロセスの mel 生成は GIL 律速**(スレッド数を増やしても 6-20 utts/s、GPU 2%) | プロセス分割が正解: 埋め込み 16→577 utts/s(28 proc)、トークン 37→**218 utts/s**(8 proc × 同一長バッチ) |
| campplus(小型 ONNX)の GPU 実行はカーネル起動律速で CPU より遅い(7 vs 16 utts/s)。さらに CUDA EP 既定の EXHAUSTIVE cudnn 探索は可変長入力で形状毎に再ベンチし 3s/utt | 小型モデルは CPU プロセス分割。CUDA EP を使う場合は `cudnn_conv_algo_search: HEURISTIC` 必須(tools は設定済み) |
| **GPU 共有プロセスへの `kill -9` が他プロセスの ORT CUDA コールを宙吊りにする**(症状: pbar 凍結 + GPU 0%、プロセスは生存) | GPU を使うプロセスの強制 kill 後は、同 GPU 上の他プロセスも再起動する。pkill -f はパターンの ssh 自己マッチに注意(PID 指定が安全) |
| **同一長バッチの数値検証**: パディング無しなら出力行=完全な出力でスライス不要。バッチ 1 参照との差は ~0.04%(1 トークン/発話以下)で、**バッチ 1 自身を 2 回実行した揺らぎと同水準**(cudnn 非決定性) | 検証はトークンレベル一致率 ≥99.5% 基準(発話単位 exact は誤検知する)。`--verify_num` が自動チェック |
| speechMOS は演技音声(ささやき・叫び)を系統的に低採点 | データセット毎に分布を実測してから閾値を決める(本データは中央値 2.18 → 閾値 1.5)。ラベル品質は cross-CER フィルタに任せる |

### 7.2 CER 評価 — 漢字混じり直接入力は読めるようになったか

`scripts/eval_ja_cer.py`: 学習データ外の漢字混じり 20 文(同形異音語・助数詞・日付・数値を含む)を 4 経路で合成し、whisper large-v3 の書き起こしと参照文の CER(NFKC + 記号除去)で比較:

| 経路 | llm | 入力 | avg CER | zero-CER 文数 |
|---|---|---|---|---|
| base_kanji | 事前学習 | 漢字直接 | **0.708(崩壊)** | 0/20 |
| base_katakana | 事前学習 | カタカナ変換 (Phase 1) | 0.097 | 9/20 |
| ft_katakana | **FT 済み** | カタカナ変換 | 0.104 | 8/20 |
| **ft_kanji** | **FT 済み** | **漢字直接** | **0.085(最良)** | 9/20 |

**結論: FT 済みモデルは漢字混じり日本語をそのまま正しく読む。**

- 事前学習モデルの漢字直接入力は完全崩壊(例:「週末は家族と温泉旅行に…」→「トウモはちゃんぞうとセイリオの…」)しており、改善は FT の効果そのもの
- ft_kanji は Phase 1 のカタカナ変換経路を**上回った**(文単位: 勝 3 / 分 16 / 負 1)。前処理なしで最良品質
- ft_kanji の「誤り」上位 3 件はすべて**表記ゆれ**(「三百キロメートル」→ whisper が「300km」と表記等)で、音声自体は正読。数値表記を正規化すれば実効 CER はさらに低い
- FT はカタカナ経路の品質を劣化させていない(0.097→0.104、ノイズ範囲)

音声サンプルは `eval_out/{path}/*.wav`、全文の書き起こしは `eval_out/report.json`。
